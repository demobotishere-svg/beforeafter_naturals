...
const Clip1: React.FC<{ src: string; frame: number; config?: any; S1: number; D1: number; playbackRate: number }> = ({ src, frame, config, S1, D1, playbackRate }) => {
  const lf = frame - S1;
  const base: Cam = {
    scale: ip(lf, [0, D1], [1.06, 1.15], EASE_GLIDE),
    tx: 0,
    ty: ip(lf, [0, D1], [15, -15], EASE_GLIDE),
    rot: ip(lf, [0, D1], [-0.4, 0.4], EASE_GLIDE),
  };
  const c = organic(base, frame, { swayPx: 12, breathe: 0.005, rotDeg: 0.8, seed: 0.3 });
  const startFrom = config ? Math.floor(config.trimStart * FPS) : 0;
  return <Video startFrom={startFrom} src={src} muted playbackRate={playbackRate} style={{ width: "100%", height: "100%", objectFit: "cover", transform: cam(c), filter: TRIX_BW }} />;
};
...
const Clip2: React.FC<{ src: string; frame: number; config?: any; S2: number; D2: number; playbackRate: number }> = ({ src, frame, config, S2, D2, playbackRate }) => {
  const lf = frame - S2;
  const base: Cam = {
    scale: ip(lf, [0, D2], [1.18, 1.05], EASE_SNAP),
    tx: ip(lf, [0, D2], [-22, 12], EASE_GLIDE),
    ty: 0,
    rot: ip(lf, [0, D2], [0.6, -0.4], EASE_GLIDE),
  };
  const c = organic(base, frame, { swayPx: 10, breathe: 0.005, rotDeg: 0.6, seed: 1.2 });
  const startFrom = config ? Math.floor(config.trimStart * FPS) : 0;
  return <OffthreadVideo startFrom={startFrom} src={src} muted playbackRate={playbackRate} style={{ width: "100%", height: "100%", objectFit: "cover", transform: cam(c), filter: TRIX_BW }} />;
};
...
const Clip3: React.FC<{ src: string; frame: number; config?: any; S3: number; D3: number; SEC: number; playbackRate: number }> = ({ src, frame, config, S3, D3, SEC, playbackRate }) => {
  ...
  const zoom = interpolate(sf, [0, 5, SEC], [1.24, 1.08, 1.06], { extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: EASE_SNAP });

  let base: Cam = { scale: 1.1, tx: 0, ty: 0, rot: 0 };
  switch (section) {
    case 0: base = { scale: zoom, tx: 0, ty: ip(sf, [0, SEC], [18, -18], EASE_GLIDE), rot: 0.4 }; break;
    case 1: base = { scale: 1.12, tx: ip(sf, [0, SEC], [28, -28], EASE_GLIDE), ty: 0, rot: -0.5 }; break;
    case 2: base = { scale: zoom, tx: 0, ty: ip(sf, [0, SEC], [-16, 16], EASE_GLIDE), rot: 0.4 }; break;
    case 3: base = { scale: 1.1, tx: ip(sf, [0, SEC], [-22, 22], EASE_GLIDE), ty: ip(sf, [0, SEC], [12, -12], EASE_GLIDE), rot: -0.4 }; break;
    case 4: base = { scale: 1.14, tx: 0, ty: ip(sf, [0, SEC], [26, -26], EASE_GLIDE), rot: 0.6 }; break;
    case 5: base = { scale: ip(sf, [0, SEC], [1.08, 1.25], EASE_ACCEL), tx: 0, ty: 0, rot: -0.5 }; break;
  }
  const c = organic(base, frame, { swayPx: 10, breathe: 0.005, rotDeg: 0.6, seed: section * 0.8 });
  const splitPx = ip(sf, [0, 6], [9, 0], EASE_ACCEL); // hard chroma fringe on cut
  const filter = section % 2 === 0 ? TRIX_BW : TRIX_CRUSH;
  const startFrom = config ? Math.floor(config.trimStart * FPS) : 0;
  return <ChromaClip startFrom={startFrom} src={src} transform={cam(c)} filter={filter} splitPx={splitPx} playbackRate={playbackRate} />;
};
...
const ColorExplosion: React.FC<{ frame: number; S4: number }> = ({ frame, S4 }) => {
...
// ── MAIN COMPOSITION ────────────────────────────────────────────────────────
export const HypeTemplate: React.FC<{ videoUrls: string[]; bgmUrl?: string; claudeConfig?: any }> = ({
  videoUrls,
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: "#000" }}>
      ...
      {videoUrls[0] && (<Sequence from={S1} durationInFrames={D1 + GW}><Clip1 src={videoUrls[0]} frame={frame} config={claudeConfig?.entering} S1={S1} D1={D1} playbackRate={r1} /></Sequence>)}
      {videoUrls[1] && (<Sequence from={S2} durationInFrames={D2 + GW}><Clip2 src={videoUrls[1]} frame={frame} config={claudeConfig?.choosing} S2={S2} D2={D2} playbackRate={r2} /></Sequence>)}
      {videoUrls[2] && (<Sequence from={S3} durationInFrames={D3 + GW}><Clip3 src={videoUrls[2]} frame={frame} config={claudeConfig?.haircut} S3={S3} D3={D3} SEC={SEC} playbackRate={r3} /></Sequence>)}
      ...
      <ColorExplosion frame={frame} S4={S4} />
